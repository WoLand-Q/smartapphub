...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="news.php"><?=__('News')?></a></li>
        <li class="breadcrumb-item active"><?= $item ? __('Edit') : __('New news') ?></li>
    </ol>
</nav>
...
<div class="card card-hover">
    <div class="card-body">
        <form method="post">
            <div class="mb-2">
                <input class="form-control" name="title" placeholder="<?=__('Title')?>" value="<?=e($item['title']??'')?>" required>
            </div>
            <div class="mb-2">
                <input class="form-control" name="tags" placeholder="<?=__('Tags (comma separated)')?>" value="<?=e($item['tags']??'')?>">
            </div>
            <div class="mb-2">
                <input class="form-control" name="author" placeholder="<?=__('Author')?>" value="<?=e($item['author']??'')?>">
            </div>
            <div class="mb-3">
            ...
            <?php if($item): ?>
                <a class="btn btn-outline-secondary ms-2" href="news_view.php?id=<?=$item['id']?>"><?=__('Preview')?></a>
            <?php endif; ?>
        </form>
    </div>
</div>
