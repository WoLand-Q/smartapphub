...
if(!$it){
    ...
    echo '<div class="text-muted">'.__('News not found').'</div>';
...
<div class="card card-hover">
    <div class="card-body">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
            ...
            <?php if(!empty($it['tags'])): foreach(explode(',', $it['tags']) as $tg): ?>
                <span class="chip"><?=e(trim($tg))?></span>
            <?php endforeach; endif; ?>
            <span class="badge bg-secondary ms-auto">
                <?=e($it['created_at'])?><?= $it['author'] ? ' · '.e($it['author']) : '' ?>
            </span>
            <?php if(is_admin()): ?>
                <a class="btn btn-sm btn-outline-primary ms-2" href="news_edit.php?id=<?=$it['id']?>"><?=__('Edit')?></a>
            <?php endif; ?>
        </div>
